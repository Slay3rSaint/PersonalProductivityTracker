...
class Calendar {
  ...
  function save ($start, $end, $txt, $color, $bg, $id=null) {
    ...
    if ($id==null) {
      ...
      $sql = "INSERT INTO `events` (`u_id`,`evt_start`, `evt_end`, `evt_text`, `evt_color`, `evt_bg`) VALUES (?,?,?,?,?,?)";
      $data = [$username, $start, $end, strip_tags($txt), $color, $bg];
    } else {
  ...
  function get ($month, $year) {
    ...
    ) AND `u_id`=?", [$start, $end, $start, $end, $start, $end, $username]);
    $events = [];
